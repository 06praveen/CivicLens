...
function UserHeaderControls() {
  ...
  if (isAuthenticated && user) {
    return (
      <div className="flex items-center gap-2 border-l border-white/20 pl-2 sm:pl-2.5">
        {user.role === "admin" && (
          <Link
            to="/admin"
            className="px-2 py-0.5 bg-saffron text-institutional-dark hover:bg-white text-[0.65rem] font-bold tracking-wider uppercase rounded-xs transition-colors shadow-xs"
          >
            Admin Portal
          </Link>
        )}
        <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-white/10 text-white text-[0.68rem] font-medium border border-white/20">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="truncate max-w-[100px] sm:max-w-[140px]">{user.full_name || user.username}</span>
        </span>
        <button
          type="button"
          onClick={logout}
          className="text-[0.68rem] font-semibold text-white/80 hover:text-saffron transition-colors"
  ...
  return (
    <div className="flex items-center gap-1.5 border-l border-white/20 pl-2 sm:pl-2.5">
      <Link
        to="/login"
        className="px-2 py-0.5 bg-saffron text-institutional-dark hover:bg-white text-[0.68rem] font-bold rounded-xs transition-colors shadow-xs"
      >
        Sign In
      </Link>
    </div>
...
export function PortalHeader() {
  ...
  const navItems = [
    ...
    { labelKey: "nav_reports" as const, path: "/reports", icon: "M19 8H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zm-3 11H8v-5h8v5zm3-7c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm-1-9H6v4h12V3z" },
  ...
  return (
    <header id="top" className="shadow-xs flex flex-col w-full z-40 select-none">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <span className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={() => setLang("en")}
                className={`py-0.5 text-[0.7rem] font-semibold transition-all relative ${lang === "en" ? "text-white font-bold" : "text-white/70 hover:text-white"
                  }`}
              ...
              <button
                type="button"
                onClick={() => setLang("hi")}
                className={`py-0.5 text-[0.7rem] font-semibold transition-all relative ${lang === "hi" ? "text-white font-bold" : "text-white/70 hover:text-white"
                  }`}
      ...
      {/* ── SECTION 3 — Dark Navy Government Navigation Bar (#00145C) ── */}
      <nav aria-label="Primary navigation" className="bg-[#00145C] border-y border-white/15 shadow-md">
          ...
          <div className={`${mobileMenuOpen ? "flex" : "hidden md:flex"} flex-col md:flex-row md:items-center justify-start overflow-x-auto py-1 md:py-0`}>
            {navItems.map((item, idx) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 px-3.5 sm:px-4 py-3 text-[0.78rem] font-semibold transition-colors border-r border-white/10 last:border-r-0 whitespace-nowrap relative ${isActive
                    ? "text-saffron font-bold bg-white/10"
                    : "text-white/90 hover:text-saffron hover:bg-white/5"
                  }`
                }
              >
                {({ isActive }) => (
                  <>
                    <svg className={`w-4 h-4 shrink-0 ${isActive ? "text-saffron" : "text-white/80"}`} fill="currentColor" viewBox="0 0 24 24">
                      <path d={item.icon} />
                    </svg>
                    <span>{t(item.labelKey)}</span>
                    {isActive && (
                      <span className="absolute bottom-0 left-0 right-0 h-[3px] bg-saffron rounded-t-sm" />
                    )}
                  </>
                )}
              </NavLink>
